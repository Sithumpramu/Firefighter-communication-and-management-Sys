...
// Trigger automatic blinking when serious thresholds exceeded
  if (!manualBlink && (temp > 20 || humidity < 20)) {
    if (!autoBlink) {
      autoBlink = true;
  ...
  float lat = gps.location.isValid() ? gps.location.lat() : lastValidLat;
  float lon = gps.location.isValid() ? gps.location.lng() : lastValidLon;
